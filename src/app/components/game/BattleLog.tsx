...
export function BattleLog() {
  ...
  const filtered = events.filter(e => e.type !== "turn");
  ...
  return (
    <div className="w-full max-w-5xl mt-6 glass p-4">
      ...
      <ScrollArea className="max-h-48 pr-2 text-xs leading-relaxed space-y-1">
        {filtered.length === 0 && <p className="text-slate-500">Aucun événement.</p>}
        {filtered.map((e, i) => (
...
function LogEntry({ entry }: { entry: any }) {
  switch (entry.type) {
    case "attack":
      return (
        <p>
          <strong>{entry.who === "player" ? "Vous" : "AI"}</strong> attaquez : -{entry.dmg} HP
        </p>
      );
    case "defend":
      return (
    ...
    case "result":
      return (
        <p className="font-semibold">
          Résultat: {entry.outcome === "win" ? "Victoire" : entry.outcome === "lose" ? "Défaite" : "Égalité"}
        </p>
      );
    default:
      return null;
